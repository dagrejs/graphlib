...
describe('json', function () {
  ...
  it('preserves the graph value, if any', function () {
    ...
    expect(rw(new Graph().setGraph({ foo: 'bar' })).graph()).toEqual({
      foo: 'bar',
    });
  ...
  it('preserves nodes', function () {
    ...
    expect(rw(new Graph().setNode('a', { foo: 'bar' })).node('a')).toEqual({
      foo: 'bar',
    });
  });
  ...
  it('preserves simple edges', function () {
    ...
    expect(rw(new Graph().setEdge('a', 'b')).edge('a', 'b')).toBe(undefined);
  ...
  it('preserves multi-edges', function () {
    ...
    g.setEdge({ v: 'a', w: 'b', name: 'foo' });
    expect(rw(g).edge('a', 'b', 'foo')).toBe(undefined);
  ...
  it('preserves parent / child relationships', function () {
    expect(rw(new Graph({ compound: true }).setNode('a')).parent('a')).toBe(
      undefined,
    );
    expect(
