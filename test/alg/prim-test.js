...
describe("alg.prim", function() {
  it("returns an empty graph for an empty input", function() {
    ...
    var g = prim(source, util.LABEL_WEIGHT_FUNC);
  ...
  it("returns a deterministic result given an optimal solution", function() {
    ...
    source.setEdge("c", "d", 20); // This edge should not be in the min spanning tree
    source.setEdge("c", "e", 60); // This edge should not be in the min spanning tree
  ...
  it("throws an Error for unconnected graphs", function() {
    ...
    expect(function() { prim(source, util.LABEL_WEIGHT_FUNC); }).to.throw();
  });
});
